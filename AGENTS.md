...
- **Custom Worker entry** `src/worker.ts` — serves prerendered assets via `@astrojs/cloudflare/handler` and handles `/api/whiteboard/*` (WebSocket sync plus legacy and read-only compatibility media routes).
...
- **Whiteboard assets** — New image/video insertion is temporarily disabled. Existing media remains readable from legacy keys `assets/{ownerKey}/{assetId}` and from the read-only compatibility namespace `boards/{boardId}/assets/{fileId}`. The board-scoped route supports GET/HEAD only; PUT/DELETE return 405. Legacy owner keys are `google:{accountId}` for saved boards and `temp:{boardId}` for unsaved boards (24h). MP4/WebM play via same-origin `/whiteboard-player`. Hub Assets is signed-in metadata in D1; the historical `library/{ownerKey}/{boards,assets}.json` objects remain read-only migration sources in R2.
- **Share codes** — KV binding `WHITEBOARD_CODES` indexes `code:{1A2B3C4D}` → board UUID (permanent for the life of the board; join also accepts leftover `1A2B`). Digit-letter pairs; join is rate-limited per IP + per code. DO stores `meta:activeCode` (mint once). Hub join + manage-panel Copy Code / Copy Link. Share-code joiners land as Editor; UUID-only stays Viewer. Group Edit (default Off) is a live draw gate: Editors can draw only when it is On. Library delete frees the KV mapping.
- **Whiteboard auth and admission** — Clerk Google sign-in via `@clerk/react` header island (Astro 7; `@clerk/astro` not yet peer-compatible). Custom Clerk Frontend API domain: `clerk.scsfoxchase.tech`. Recents / Library / Assets are **cloud-only** D1 metadata (no localStorage board library); historical R2 JSON indexes are retained as read-only migration sources. Signed-out create is a live scratch board (ephemeral Owner via host secret) and expires in 24h if never saved. Host proof is first-message `wb:auth` / `X-Board-Host`, not the WebSocket query string; a header alone cannot claim a random UUID. `wb:auth` is repeatable: a tab that is signed in but has no JWT yet sends `signedIn: true` with no token, the DO keeps that socket pending (one `wb:hello` per socket, never a Viewer hello for a real Owner), and a token arriving later upgrades the role via `wb:role`. Excalidraw mounts before hello and starts view-only so the scene paints while Clerk loads. Worker verifies sessions with `@clerk/backend` for cloud library APIs and legacy `google:*` asset writes. Layered edge admission allows 600 connections per 60 seconds per trusted `CF-Connecting-IP` and 240 per 60 seconds per canonical board plus IP; the local/test fallback enforces both layers, expires buckets, and is capped at 4096 keys. Each DO caps total sockets at 64, pending auth at 32, and expires pending auth after about 30 seconds without a per-socket alarm. Roles: Owner, Manager, Editor, Viewer. Voluntary Follow (eyes) unfollows on pan; Follow User locks the guest camera (snap to leader bounds + overlay). Scene writes use mutation IDs/acks with bounded reconnect retry and UTF-8 byte limits; malformed/oversized/persistence errors are explicit `wb:error` frames. Structured logs are sampled at 5% and omit IDs, IPs, URLs, credentials, exceptions, and scene data.
...
| `/api/whiteboard/boards/:uuid/assets/:fileId` | `src/worker.ts` → R2 | Read-only compatibility media (GET/HEAD; PUT/DELETE return 405; UUID or SHA-256 file id) |
...
| `/api/whiteboard/library/boards` | `src/worker.ts` → D1 | Signed-in cloud board metadata (Clerk) |
| `/api/whiteboard/library/assets` | `src/worker.ts` → D1 | Signed-in cloud asset metadata (Clerk) |
...
- `wrangler.jsonc` — Worker `scsfoxchase-tech`, assets `./dist/client`, `main` `./src/worker.ts`, DO binding `WHITEBOARDS`, R2 binding `WHITEBOARD_ASSETS`, KV binding `WHITEBOARD_CODES`, D1 binding `WHITEBOARD_LIBRARY`, and layered `WHITEBOARD_CONNECT_LIMITER` / `WHITEBOARD_BOARD_CONNECT_LIMITER` bindings
