...
- `wrangler.jsonc` — Worker `scsfoxchase-tech`, assets `./dist/client`, `main` `./src/worker.ts`, DO binding `WHITEBOARDS`, R2 binding `WHITEBOARD_ASSETS`, KV binding `WHITEBOARD_CODES`, D1 binding `WHITEBOARD_LIBRARY`, and `WHITEBOARD_CONNECT_LIMITER`
