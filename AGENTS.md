...
- **Share codes** — KV binding `WHITEBOARD_CODES` indexes `code:{1A2B}` → board UUID (TTL 12h). Four-character digit-letter (`([0-9][A-Z]){2}`). DO stores `activeCode` + alarm for Open/Closed; hub join + manage-panel Open/Closed/Copy/New code.
