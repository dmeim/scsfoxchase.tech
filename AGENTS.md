...
- **Share codes** — KV binding `WHITEBOARD_CODES` indexes `code:{A1B2C3D4}` → board UUID (TTL 12h). Eight-character letter-digit (`([A-Z][0-9]){4}`). DO stores `activeCode` + alarm for Open/Closed; hub join + manage-panel Open/Closed/Copy/New code. Join is view-only unless Group Edit is On (share-code joiners land as Editor) or Owner/Manager sets Editor on People. UUID-only stays Viewer.
