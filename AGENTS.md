...
- **Share codes** — KV binding `WHITEBOARD_CODES` indexes `code:{1A2B}` → board UUID (TTL 12h). Four-character digit-letter (`([0-9][A-Z]){2}`, `1A2B` form); join is rate-limited per IP + per code. DO stores `activeCode` + alarm for Open/Closed; hub join + manage-panel Open/Closed/Copy/New code. Join is view-only unless Group Edit is On (share-code joiners land as Editor) or Owner/Manager sets Editor on People. UUID-only stays Viewer.
