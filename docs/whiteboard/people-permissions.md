...
A share code (or board UUID) only **opens** the board. Role is decided on connect. Share-code joiners land as **Viewer** unless **Group Edit** is On (`meta:classCanEdit`; manage-panel switch) — then they land as **Editor**. UUID-only stays **Viewer**. Owner or Manager can still set **Editor** on **People**. A join code alone does not mean students can draw.
...
- Default for new guest connects: **Viewer** (UUID-only always). Share-code joiners land as **Editor** when **Group Edit** is On.
...
| View only | Viewer (default for guests and UUID-only links; code join unless Group Edit is On) |
| Grant/revoke Manager | Owner only |
| Set Editor / Viewer | Owner or Manager (per person on People) |
| Group Edit | Owner or Manager. When On, share-code joiners land as Editor. UUID-only stays Viewer. Default Off. |
