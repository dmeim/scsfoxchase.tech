...
The Worker validates the board UUID, canonical UUID `sessionId`, and WebSocket upgrade before resolving the board Durable Object. `WHITEBOARD_CONNECT_LIMITER` admits 600 upgrades per 60 seconds per trusted `CF-Connecting-IP`, and `WHITEBOARD_BOARD_CONNECT_LIMITER` admits 240 per 60 seconds per canonical board UUID plus trusted IP. Local/test fallback buckets enforce both layers, expire, and are capped at 4096 keys. A board caps total sockets at 64 and pending authentication sockets at 32. Pending auth expires after approximately 30 seconds without creating a per-socket alarm.
